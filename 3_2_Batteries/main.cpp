...
struct Bank
{
    ...
    {
        std::string highJoltage{};
        const static size_t MAX_DIGITS_IN_HIGHT_JOLTAGE = 12;
        size_t distance = 0;
        for (size_t i=0; i<MAX_DIGITS_IN_HIGHT_JOLTAGE; i++)
        {
            std::vector<Battery>::const_iterator max_element = std::max_element(batteries.cbegin()+distance, 
                                                                                batteries.cend()-MAX_DIGITS_IN_HIGHT_JOLTAGE+i+1);
            highJoltage += std::string(1, max_element->id);
            distance = std::distance(batteries.cbegin(), max_element) + 1;
            // cout << "max_element: " << max_element->id << " at distance: " << distance << " ";
        }
        cout << "number: " << highJoltage << "\n";

        return std::stol(highJoltage);
    }
...
struct Lobby
{
    ...
    long long GetAccumulatedLargestJoltageFromAllBanks()
    {
        return std::accumulate(this->banks.cbegin(), this->banks.cend(), 0LL, [](long long sum, const Bank &bank)
                                {
                                    long long val = bank.GetLargestJoltage();
                                    // cout << val << "\n";
                                    return sum + val;
                                });
    }
...
int main()
{
    ...
    // string filename("../3_1_Batteries/example_input"); // 3121910778619
    string filename("../3_1_Batteries/input"); // 173416889848394
