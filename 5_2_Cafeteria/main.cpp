...
struct IngredientRange
{
    ...
    {
        return upperBound.id - lowerBound.id + 1;
    }
    bool IsFresh(const Ingredient& ingredient) const
    {
        return (ingredient.id >= this->lowerBound.id) && (ingredient.id <= this->upperBound.id);
    }
    bool Contains(const IngredientRange& ingredientRange) const
    {
        return (ingredientRange.lowerBound.id >= this->lowerBound.id) && (ingredientRange.upperBound.id <= this->upperBound.id);
        // return this->lowerBound.id <= ingredientRange.upperBound.id;
    }
    bool Extends(const IngredientRange& ingredientRange) const
    {
        return (ingredientRange.lowerBound.id <= this->upperBound.id) && (ingredientRange.upperBound.id > this->upperBound.id);
        // return this->lowerBound.id <= ingredientRange.upperBound.id;
    }

    bool operator<(const IngredientRange &ingredientRange) const
    {
        // return (this->lowerBound.id <= ingredientRange.lowerBound.id) && (this->upperBound.id <= ingredientRange.upperBound.id);
        return (this->lowerBound.id < ingredientRange.lowerBound.id) ? true :
                (this->lowerBound.id == ingredientRange.lowerBound.id) ? (this->upperBound.id <= ingredientRange.upperBound.id) :
                false;
    }
...
struct Cafeteria
{
    ...
    {
        std::sort(freshIngredients.begin(), freshIngredients.end(), std::less<IngredientRange>());
        freshIngredientsCleaned.emplace_back(freshIngredients.front());
        for (const auto& range : freshIngredients)
        {
            if (freshIngredientsCleaned.back().Contains(range))
            {
                continue;
            }
            if (freshIngredientsCleaned.back().Extends(range))
            {
                // cout << "(" << range.lowerBound.id << "," << range.upperBound.id << ") contains: (" <<
                //         freshIngredientsCleaned.back().lowerBound.id << "," << freshIngredientsCleaned.back().upperBound.id << ")\n";
                freshIngredientsCleaned.back().upperBound = range.upperBound;
                continue;
            }
            freshIngredientsCleaned.emplace_back(range);

        }
    }

    long long GetNumberOfTotalFreshIngredients()
    {
        return std::accumulate(this->freshIngredientsCleaned.cbegin(), this->freshIngredientsCleaned.cend(), 0LL, [this](long long sum, const IngredientRange &ingredientRange)
                                {
                                    sum += ingredientRange.size();
                                    cout << sum << " (+" << ingredientRange.size() << ")\n";
                                    return sum;
                                });
    }
    ...
    static std::string const ToString2(const Cafeteria &cafeteria)
    {
        return boost::algorithm::join(cafeteria.freshIngredientsCleaned |
                    boost::adaptors::transformed([cafeteria](const IngredientRange &ingredientRange)
                    {
                        return IngredientRange::ToString(ingredientRange);
                    }), "\n");
...
int main()
{
    ...
    // string filename("../5_1_Cafeteria/example_input"); // 14
    string filename("../5_1_Cafeteria/input"); // 344486348901788
    // 301346528436430 is too low 
    // 334569956044047 is too low
    // 334569956044067 is too low
    ...
    std::cout << Cafeteria::ToString(cafeteria) << "\n\n";
    std::cout << Cafeteria::ToString2(cafeteria) << "\n";
    // std::cout << "GetNumberOfFreshIngredients: " << cafeteria.GetNumberOfFreshIngredients() << "\n";
    std::cout << "GetNumberOfFreshIngredients: " << cafeteria.GetNumberOfTotalFreshIngredients() << "\n";
