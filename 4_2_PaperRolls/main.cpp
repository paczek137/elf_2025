...
struct GridNode
{
    const static size_t MAX_ROLLS = 1;
    ...
    {
        // cout << "(r:" << rolls << ",d:" << distance << ")";
        if (rolls >= (GridNode::MAX_ROLLS)) { return false; }
        if ((left == nullptr) || (distance == 0)) { /*std::cout << " null or dist=0 ";*/ return true; }
        if (left->IsEmpty()) { /*std::cout << " empty ";*/ return left->HasLeftNeighbourFree(distance-1, rolls); }
        // std::cout << " roll ";
        return left->HasLeftNeighbourFree(distance-1, rolls+1);
    }
    bool HasRightNeighbourFree(size_t distance, size_t rolls = 0)
    {
        if (rolls >= (GridNode::MAX_ROLLS)) { return false; }
        if ((right == nullptr) || (distance == 0)) { return true; }
        if (right->IsEmpty()) { return right->HasRightNeighbourFree(distance-1, rolls); }
        return right->HasRightNeighbourFree(distance-1, rolls+1);
    }
    bool HasUpperNeighbourFree(size_t distance, size_t rolls = 0)
    {
        if (rolls >= (GridNode::MAX_ROLLS)) { return false; }
        if ((up == nullptr) || (distance == 0)) { return true; }
        if (up->IsEmpty()) { return up->HasUpperNeighbourFree(distance-1, rolls); }
        return up->HasUpperNeighbourFree(distance-1, rolls+1);
    }
    bool HasDownNeighbourFree(size_t distance, size_t rolls = 0)
    {
        if (rolls >= (GridNode::MAX_ROLLS)) { return false; }
        if ((down == nullptr) || (distance == 0)) { return true; }
        if (down->IsEmpty()) { return down->HasDownNeighbourFree(distance-1, rolls); }
        return down->HasDownNeighbourFree(distance-1, rolls+1);
    }
    bool HasUpperLeftNeighbourFree(size_t distance, size_t rolls = 0)
    {
        if (rolls >= (GridNode::MAX_ROLLS)) { return false; }
        if ((up == nullptr) || (up->left == nullptr) || (distance == 0)) { return true; }
        if (up->left->IsEmpty()) { return up->left->HasUpperLeftNeighbourFree(distance-1, rolls); }
        return up->left->HasUpperLeftNeighbourFree(distance-1, rolls+1);
    }
    bool HasUpperRightNeighbourFree(size_t distance, size_t rolls = 0)
    {
        if (rolls >= (GridNode::MAX_ROLLS)) { return false; }
        if ((up == nullptr) || (up->right == nullptr) || (distance == 0)) { return true; }
        if (up->right->IsEmpty()) { return up->right->HasUpperRightNeighbourFree(distance-1, rolls); }
        return up->right->HasUpperRightNeighbourFree(distance-1, rolls+1);
    }
    bool HasDownLeftNeighbourFree(size_t distance, size_t rolls = 0)
    {
        if (rolls >= (GridNode::MAX_ROLLS)) { return false; }
        if ((down == nullptr) || (down->left == nullptr) || (distance == 0)) { return true; }
        if (down->left->IsEmpty()) { return down->left->HasDownLeftNeighbourFree(distance-1, rolls); }
        return down->left->HasDownLeftNeighbourFree(distance-1, rolls+1);
    }
    bool HasDownRightNeighbourFree(size_t distance, size_t rolls = 0)
    {
        if (rolls >= (GridNode::MAX_ROLLS)) { return false; }
        if ((down == nullptr) || (down->right == nullptr) || (distance == 0)) { return true; }
        if (down->right->IsEmpty()) { return down->right->HasDownRightNeighbourFree(distance-1, rolls); }
        return down->right->HasDownRightNeighbourFree(distance-1, rolls+1);
    }
...
struct Grid
{
    ...
    {
        const static size_t DISTANCE = 1;
        const static size_t MAX_ROLLS_COUNT = 4;
        size_t i=0;
        for (auto &el : allGridNodes)
        {
            // cout << "(x: " << el.x << ", y: " << el.y << "): " << el.symbol << "\n";
            // cout << i++ << ": ";
            if(el.IsEmpty()) { continue; }
            // if(!el.HasLeftNeighbourFree(DISTANCE)) { /*std::cout << "\n";*/ continue; }
            // if(!el.HasRightNeighbourFree(DISTANCE)) { continue; }
            // if(!el.HasUpperNeighbourFree(DISTANCE)) { continue; }
            // if(!el.HasDownNeighbourFree(DISTANCE)) { continue; }
            // if(!el.HasUpperLeftNeighbourFree(DISTANCE)) { continue; }
            // if(!el.HasUpperRightNeighbourFree(DISTANCE)) { continue; }
            // if(!el.HasDownLeftNeighbourFree(DISTANCE)) { continue; }
            // if(!el.HasDownRightNeighbourFree(DISTANCE)) { continue; }
            
            size_t rollsCounter = 0;
            if(!el.HasLeftNeighbourFree(DISTANCE)) { rollsCounter++; }
            if(!el.HasRightNeighbourFree(DISTANCE)) { rollsCounter++; }
            if(!el.HasUpperNeighbourFree(DISTANCE)) { rollsCounter++; }
            if(!el.HasDownNeighbourFree(DISTANCE)) { rollsCounter++; }
            if(!el.HasUpperLeftNeighbourFree(DISTANCE)) { rollsCounter++; }
            if(!el.HasUpperRightNeighbourFree(DISTANCE)) { rollsCounter++; }
            if(!el.HasDownLeftNeighbourFree(DISTANCE)) { rollsCounter++; }
            if(!el.HasDownRightNeighbourFree(DISTANCE)) { rollsCounter++; }

            if (rollsCounter >= MAX_ROLLS_COUNT) continue;
    ...
    long long GetNumberOfAllAccessableGridNodes()
    {
